...
class TestConsensusEngine:
    ...
    def load_data(self):
        ...
        logger.info(f"✓ Dataset loaded: {data['X_train'].shape[0]} train samples")
        return data
        ...
        for sample_idx in range(min(3, len(X_test))):
            ...
            for agent_name, agent in agents.items():
                if not agent.is_trained:
                    logger.info(f"  {agent_name:25s}: ⊘ Skipped (not trained)")
                    continue
                
                try:
                    prediction, confidence = agent.predict(X_sample)
                    prediction_label = 'SPAM' if prediction == 1 else 'HAM'
                    correct = "✓" if prediction == y_true else "✗"
    ...
    def test_consensus_predictions(self, engine: ConsensusEngine, X_test: np.ndarray, y_test: np.ndarray):
        ...
        for sample_idx in range(min(3, len(X_test))):
            ...
            try:
                ...
                correct = "✓" if result.predicted_class == y_true else "✗"
